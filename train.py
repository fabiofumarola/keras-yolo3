...
@click.command()
@click.option('--annotation_path', required=True,
              help='the path of the file with the image paths and annotations')
@click.option('--log_dir',
              help='the directory for the results and log')
@click.option('--classes_path', required=True,
              help='the path with the classes for the model')
@click.option('--anchors_path', required=True,
              help='the path with the paths for the model')
@click.option('--weights_path', required=True,
              help="""the path of the model to use as starting point,
             specify None to start from scratch""")
@click.option('--mode',  type=click.Choice(['only_dense', 'fine_tuning']))
def _main(annotation_path, log_dir, classes_path, anchors_path, weights_path,
          mode):
    np.random.seed(42)
...
def create_model(input_shape, anchors, num_classes, weights_path, freeze_body=2):
...
def create_tiny_model(input_shape, anchors, num_classes, weights_path,
                      freeze_body=2):
